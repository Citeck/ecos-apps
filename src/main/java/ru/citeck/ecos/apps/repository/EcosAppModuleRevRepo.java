...
@Repository
public interface EcosAppModuleRevRepo extends JpaRepository<EcosAppModuleRevEntity, Long> {
    ...
    @Query("SELECT m FROM EcosAppModuleRevEntity m WHERE m.moduleId = ?1 ORDER BY m.id DESC")
    EcosAppModuleRevEntity getLastRevisionByAppId(Long id);
}
