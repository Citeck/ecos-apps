...
timestamps {
  node {
    ...
    def repoUrl = "git@bitbucket.org:citeck/ecos-apps.git"
    ...
    try {
      ...
      stage('Build project artifacts') {
        withMaven(mavenLocalRepo: '/opt/jenkins/.m2/repository', tempBinDir: '') {
          sh "mvn clean package -Dskip.npm -Pprod"
          // build-info
          ...
          // /build-info
          sh "mvn jib:dockerBuild -Dskip.npm -Pprod -Djib.docker.image.tag=${project_version}"
        }
